#include "myPenaltyInterfaceDiffusion.h"
#include <cmath>

registerMooseObject("testthingApp", myPenaltyInterfaceDiffusion);

InputParameters
myPenaltyInterfaceDiffusion::validParams()
{
  ...
  params.addRequiredCoupledVar("cl","concentration");
...
myPenaltyInterfaceDiffusion::myPenaltyInterfaceDiffusion(const InputParameters & parameters)
...
Real
myPenaltyInterfaceDiffusion::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0;
...
Real
myPenaltyInterfaceDiffusion::computeQpJacobian(Moose::DGJacobianType type)
{
  Real jac = 0;
